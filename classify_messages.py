...
def main():
    ...
    # Ensure usefulness_score is properly formatted as float between 0 and 1 with 4 decimal places
    df['usefulness_score'] = pd.to_numeric(df['usefulness_score'], errors='coerce').clip(0, 1).round(4)
    ...
    # Save classified messages with proper float formatting
    df.to_csv('telegram_messages_classified.csv', index=False, float_format='%.4f')
    
    # Save labeled messages (only those with labels) and sort by usefulness score
    labeled_df = df[df['labels'].apply(lambda x: len(x) > 0)].copy()
    labeled_df = labeled_df.sort_values('usefulness_score', ascending=False)
    labeled_df.to_csv('telegram_messages_labeled.csv', index=False, float_format='%.4f')
    
    # Save formatted Excel files
    df.to_excel('telegram_messages_classified.xlsx', index=False)
    labeled_df.to_excel('telegram_messages_labeled.xlsx', index=False)
